<?php
	...
	class Board extends CI_Controller
	{
		...
		{	
			...
			$data["home"] = $home;
			$data['title'] = 'Add New Board Member';
			$data['message'] = $this->session->flashdata('message');
			// generate pagination
			$this->load->library('pagination');
			$config['base_url'] = site_url('admin/board/index/');
			...
			$this->load->view('admin/header',$data);
		 	$this->load->view('admin/board/home',$data);
		...
		function add()
		{
			...
			$session_data = $this->session->userdata('logged_in');
		 	$data['email'] = $session_data['email'];
			$data['session_data'] = $session_data;
			$data['title'] = 'Add New Board Memember';
			$data["ckeditor"] = $this->data['ckeditor'];
			$data['action'] = site_url('admin/board/addbanner');
			$data['error'] = "";
			
			// load view
			$this->load->view('admin/header',$data);
		 	$this->load->view('admin/board/homeAdd', $data);
		...
		function addbanner()
		{
			...
			else
			{
				...
				$this->load->library('upload', $config);
					...
					// save data
					$home = array('content' => $this->input->post('content'),'status' => $this->input->post('status'),'image' => ($val["upload_data"]["orig_name"]));   
					$this->load->model("boardModel");
					...
					$home = array('content' => $this->input->post('content'),'status' => $this->input->post('status'),'img' => (""));
		...
		{	
			...
			$data['action'] = site_url('admin/board/update/');
			$data['error'] = "";
			
		 	$this->load->view('admin/header',$data);
		 	$this->load->view('admin/board/homeEdit',$data);
			$this->load->view('admin/footer');
			...
			{	
				...
				if($_REQUEST["img"] == "" && isset($_FILES) && $_FILES["userfile"]["name"] !=""){ 
					...
					{	
						...
						$this->load->model("boardModel");
						$home = $this->boardModel->get_by_id($id)->row();						
						$data["home"] = $home;
						$data['action'] = site_url('admin/board/update/');
						$this->load->view('admin/header',$data);
						$this->load->view('admin/board/homeEdit',$data);
						...
						$id = $this->input->post('hid');
						$home = array('title' => $this->input->post('content'),'status' => $this->input->post('status'),'image' => ($val["upload_data"]["orig_name"]));
						$this->boardModel->update($id,$home);
				...
				$id = $this->input->post('hid');
				$home = array('content' => $this->input->post('content'),'status' => $this->input->post('status'));
				$this->boardModel->update($id,$home);
		...
		{			
			...
			foreach($home as $item) { 
				...
				$this->boardmodel->update_banner($home[$i]->hid,$arr);
